...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    setLoading(true);
    try {
      const result = await authService.register({ name: data.name, email: data.email, password: data.password });
      setUser(result.user);
      toast.success(`Welcome to Snappad, ${result.user.name}!`);
      navigate('/dashboard');
    } catch (err: any) {
  ...
  useEffect(() => {
    const onSuccess = (e: Event) => {
      ...
      setUser(data.user);
      toast.success(`Welcome to Snappad, ${data.user.name}!`);
      navigate('/dashboard');
    };
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Create your vault</p>
        </div>
